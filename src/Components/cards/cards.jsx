import './cards.css'
export default function Cards() {
    return (
        <>
            <section class="section-card">
                <div class="row">
                    <div class="card-1">
                        <img src="\image\Cards\Card-1.jpg" alt="img" />
                        <h4 class="card-t">Facilidad de creación</h4>
                        <p class="card-t1">Crea campeonatos en cuestión de minutos con nuestro proceso intuitivo y amigable.</p>
                    </div>
                    <div class="card-2">
                        <img src="\image\Cards\Card-2.jpg" alt="img" />
                        <h4 class="card-t">Sorteo automático de equipos</h4>
                        <p class="card-t1">Di adiós a las complicaciones. Deja que nuestro sistema inteligente distribuya los equipos de manera justa y aleatoria.</p>
                    </div>
                    <div class="card-1">
                        <img src="\image\Cards\Card-3.jpg" alt="img" />
                        <h4 class="card-t">Cronogramas personalizados</h4>
                        <p class="card-t1">Elabora tus cronogramas flexibles y personalizables para adaptarse a cualquier estilo de campeonato.</p>
                    </div>
                </div>
                <div class="row">
                    <div class="card-2">
                        <img src="\image\Cards\Card-4.jpg" alt="img" />
                        <h4 class="card-t">Estadísticas en tiempo real</h4>
                        <p class="card-t1">Crea campeonatos en cuestión de minutos con nuestro proceso intuitivo y amigable.</p>
                    </div>
                    <div class="card-1">
                        <img src="\image\Cards\Card-5.jpg" alt="img" />
                        <h4 class="card-t">Visualiza participantes</h4>
                        <p class="card-t1">Explora quiénes son todos los participantes y capitanes de los equipos.</p>
                    </div>
                    <div class="card-2">
                        <img src="\image\Cards\Card-6.jpg" alt="img" />
                        <h4 class="card-t">  Comparte los mejores momentos</h4>
                        <p class="card-t1">Comparte los momentos inolvidables del campeonato con la posibilidad de agregar fotos que serán visibles para tus jugadores.</p>
                    </div>
                </div>
            </section>
        </>
    )
}